...
public class PlayerController : MonoBehaviour
{
    ...
    void Move_Update()
    {
        ...
        {
            hVelocity = playerTransform.forward * Input.GetAxis("Vertical") + 
    ...
    void View_Update()
    {
        if(Input.GetMouseButton(0)) // if fire
        {
            // rotate around y axis 
            rotationY += Input.GetAxis("Mouse X") * rotationSensitivity * Time.deltaTime + Random.Range(-0.05f, 0.05f);
        
            // rotate around x axis
            rotationX += Input.GetAxis("Mouse Y") * rotationSensitivity * Time.deltaTime + Random.Range(-0.05f, 0.06f);
